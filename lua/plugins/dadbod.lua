  ...
  config = function()
    ...
    vim.api.nvim_create_autocmd('FileType', {
      ...
      callback = function(event)
        map('n', '<leader>Dr', '<Cmd>DBUIExecute<CR>', { desc = 'Execute Query', buffer = event.buf, table.unpack(opts) })
        map('v', '<leader>Dr', '<Cmd>DBUIExecute<CR>',
          { desc = 'Execute Selection', buffer = event.buf, table.unpack(opts) })
        map('n', '<leader>DS', '<Cmd>DBUISaveQuery<CR>', { desc = 'Save Query', buffer = event.buf, table.unpack(opts) })
    ...
    vim.api.nvim_create_user_command('DBStatus', function()
      ...
      vim.notify(is_db_buf and 'Connected to database' or 'Not in a database buffer',
