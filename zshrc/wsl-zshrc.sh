...
plugins=(git z colored-man-pages command-not-found zsh-autosuggestions zsh-syntax-highlighting sudo copypath copyfile jsontools web-search)
...
setopt AUTO_CONTINUE LONG_LIST_JOBS NOTIFY
...
gacp() { [[ -z "$1" ]] && { echo "Usage: gacp <commit-message>"; return 1; }; git add --all && git commit -m "$1" && git push; }
...
weather() { local location="${1:-}"; curl -s "http://wttr.in/${location}"; }

qr() { [[ -z "$1" ]] && { echo "Usage: qr <text>"; return 1; }; curl -s "http://qrenco.de/$1"; }
...
webserver() { local port="${1:-8000}"; echo "Starting web server on port $port..."; python3 -m http.server "$port"; }
...
testtools() {
    local tools=("git" "starship" "zoxide" "fzf" "eza" "bat" "fd" "rg" "nvim""explorer.exe")
...
sysinfo() {
    ...
    echo "Editor: $EDITOR"
    if command -v starship &>/dev/null; then
        echo "Prompt: Starship $(starship --version | head -1)"
    fi
    if [[ -n "$WSL_DISTRO_NAME" ]]; then
...
if [[ -n "$WSL_DISTRO_NAME" ]]; then
    wpath() { [[ -z "$1" ]] && { echo "Usage: wpath <linux-path>"; return 1; }; wslpath -w "$(realpath "$1")"; }
    lpath() { [[ -z "$1" ]] && { echo "Usage: lpath <windows-path>"; return 1; }; wslpath "$1"; }
    winopen() {
        local target="${1:-.}"
        if [[ -d "$target" ]]; then
